...
func TestStrict(t *testing.T) {
	...
	runTests(t, relaxedDsn, func(dbt *DBTest) {
		...
		var queries = [...]struct {
			...
			{"INSERT INTO test VALUES(10,'sql'),(NULL,'test'),(300,'Open Source')", []string{"1265", "1048", "1264", "1265"}},
		}
		var err error
...
func TestCustomDial(t *testing.T) {
	...
	db, err := sql.Open("sql", fmt.Sprintf("%s:%s@mydial(%s)/%s?timeout=30s&strict=true", user, pass, addr, dbname))
	if err != nil {
		t.Fatalf("error connecting: %s", err.Error())
	}
	defer db.Close()
